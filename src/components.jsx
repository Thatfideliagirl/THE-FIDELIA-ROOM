    ...
    <div className="min-h-screen flex items-center justify-center px-6 py-10">
      <div className="reveal in w-full max-w-[560px]">
        ...
        <div className="card rounded-2xl p-8" style={{ boxShadow: "0 20px 50px -24px rgba(38,32,25,0.16)" }}>
          {step === 0 && <div className="flex flex-col gap-4"><h2 className="f-display text-[26px] mb-1" style={{ fontWeight: 800 }}>Let's start with you.</h2><Field label="Full name" value={name} onChange={(e) => setName(e.target.value)} /><Field label="Email address" value={email} onChange={(e) => setEmail(e.target.value)} /><Field label="Phone / WhatsApp (required, so we can always reach you)" value={phone} onChange={(e) => setPhone(e.target.value)} /><div><Field label="Password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="At least 6 characters" />{password.length > 0 && password.length < 6 && <div className="text-[12px] mt-1.5" style={{ color: "#B04A3A" }}>Needs at least 6 characters ({password.length}/6 so far).</div>}</div><button disabled={!name.trim() || !email.trim() || !phone.trim() || password.length < 6} onClick={() => setStep(1)} className="btn-primary rounded-lg py-3 text-[15px] mt-2">Continue</button></div>}
          ...
          {step === 2 && <div className="flex flex-col gap-4"><h2 className="f-display text-[26px] mb-1" style={{ fontWeight: 800 }}>A few questions.</h2>{questions.map((q, i) => <TextArea key={i} label={q} value={answers[i]} onChange={(e) => setAnswers((a) => a.map((x, idx) => idx === i ? e.target.value : x))} />)}{questions.length > 0 && answers.some((a) => !a.trim()) && <div className="text-[12px]" style={{ color: "#B04A3A" }}>Please answer every question before continuing.</div>}<div className="flex gap-3 mt-1"><button onClick={() => setStep(1)} className="text-[13px]" style={{ color: "#A79B84" }}>Back</button><button disabled={answers.some((a) => !a.trim())} onClick={() => setStep(3)} className="btn-primary rounded-lg py-3 text-[15px] flex-1">Review application</button></div></div>}
...
export function InReviewScreen({ applicant, onExit, exitLabel = "SIGN OUT" }) { return <div className="min-h-screen flex items-center justify-center px-6 text-center"><div className="reveal in max-w-[420px]"><LogoMark height={64} /><div className="f-label text-[13px] mt-6 mb-3 accent-text">APPLICATION IN REVIEW</div><h1 className="f-display text-[30px] mb-4" style={{ fontWeight: 800 }}>Thanks, {applicant.name.split(" ")[0]}.</h1><p className="text-[15px] leading-relaxed mb-8" style={{ color: "#71675A" }}>Your application is being reviewed by the Room. Keep an eye on your email for your code — including your spam/junk folder, just in case.</p><button onClick={onExit} className="f-label text-[12px]" style={{ color: "#A79B84" }}>{exitLabel}</button></div></div>; }
